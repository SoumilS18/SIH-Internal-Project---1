"""
Generates exhaustive Hindi mappings for all 786 districts in districtsCatalog.json.
"""
import json

# Comprehensive official Hindi names for Indian districts across all states
MANUAL_DISTRICT_HINDI = {
    # Uttar Pradesh (75 districts)
    "agra": "आगरा",
    "aligarh": "अलीगढ़",
    "ambedkar nagar": "अंबेडकर नगर",
    "amethi": "अमेठी",
    "amroha": "अमरोहा",
    "auraiya": "औरैया",
    "ayodhya": "अयोध्या",
    "azamgarh": "आजमगढ़",
    "baghpat": "बागपत",
    "bahraich": "बहराइच",
    "ballia": "बलिया",
    "balrampur": "बलरामपुर",
    "banda": "बांदा",
    "barabanki": "बाराबंकी",
    "bareilly": "बरेली",
    "basti": "बस्ती",
    "bhadohi": "भदोही",
    "bijnor": "बिजनौर",
    "budaun": "बदायूँ",
    "bulandshahr": "बुलंदशहर",
    "chandauli": "चंदौली",
    "chitrakoot": "चित्रकूट",
    "deoria": "देवरिया",
    "etah": "एटा",
    "etawah": "इटावा",
    "farrukhabad": "फर्रुखाबाद",
    "fatehpur": "फतेहपुर",
    "firozabad": "फिरोजाबाद",
    "gautam buddha nagar": "गौतम बुद्ध नगर",
    "ghaziabad": "गाजियाबाद",
    "ghazipur": "गाजीपुर",
    "gonda": "गोंडा",
    "gorakhpur": "गोरखपुर",
    "hamirpur": "हमीरपुर",
    "hapur": "हापुड़",
    "hardoi": "हरदोई",
    "hathras": "हाथरस",
    "jalaun": "जालौन",
    "jaunpur": "जौनपुर",
    "jhansi": "झांसी",
    "kannauj": "कन्नौज",
    "kanpur dehat": "कानपुर देहात",
    "kanpur nagar": "कानपुर नगर",
    "kasganj": "कासगंज",
    "kaushambi": "कौशाम्बी",
    "kushinagar": "कुशीनगर",
    "lakhimpur kheri": "लखीमपुर खीरी",
    "lalitpur": "ललितपुर",
    "lucknow": "लखनऊ",
    "maharajganj": "महराजगंज",
    "mahoba": "महोबा",
    "mainpuri": "मैनपुरी",
    "mathura": "मथुरा",
    "mau": "मऊ",
    "meerut": "मेरठ",
    "mirzapur": "मिर्जापुर",
    "moradabad": "मुरादाबाद",
    "muzaffarnagar": "मुजफ्फरनगर",
    "pilibhit": "पीलीभीत",
    "pratapgarh": "प्रतापगढ़",
    "prayagraj": "प्रयागराज",
    "raebareli": "रायबरेली",
    "rampur": "रामपुर",
    "saharanpur": "सहारनपुर",
    "sambhal": "संभल",
    "sant kabir nagar": "संत कबीर नगर",
    "shahjahanpur": "शाहजहांपुर",
    "shamli": "शामली",
    "shravasti": "श्रावस्ती",
    "siddharthnagar": "सिद्धार्थनगर",
    "sitapur": "सीतापुर",
    "sonbhadra": "सोनभद्र",
    "sultanpur": "सुल्तानपुर",
    "unnao": "उन्नाव",
    "varanasi": "वाराणसी",

    # Madhya Pradesh (55 districts)
    "agar malwa": "आगर मालवा",
    "alirajpur": "अलीराजपुर",
    "anuppur": "अनूपपुर",
    "ashoknagar": "अशोकनगर",
    "balaghat": "बालाघाट",
    "barwani": "बड़वानी",
    "betul": "बैतूल",
    "bhind": "भिंड",
    "bhopal": "भोपाल",
    "burhanpur": "बुरहानपुर",
    "chhatarpur": "छतरपुर",
    "chhindwara": "छिंदवाड़ा",
    "damoh": "दमोह",
    "datia": "दतिया",
    "dewas": "देवास",
    "dhar": "धार",
    "dindori": "डिंडोरी",
    "guna": "गुना",
    "gwalior": "ग्वालियर",
    "harda": "हरदा",
    "hoshangabad": "नर्मदापुरम (होशंगाबाद)",
    "narmadapuram": "नर्मदापुरम",
    "indore": "इंदौर",
    "jabalpur": "जबलपुर",
    "jhabua": "झाबुआ",
    "katni": "कटनी",
    "khandwa": "खंडवा",
    "khargone": "खरगोन",
    "mandla": "मंडला",
    "mandsaur": "मंदसौर",
    "morena": "मुरैना",
    "narsinghpur": "नरसिंहपुर",
    "neemuch": "नीमच",
    "niwari": "निवाड़ी",
    "panna": "पन्ना",
    "raisen": "रायसेन",
    "rajgarh": "राजगढ़",
    "ratlam": "रतलाम",
    "rewa": "रीवा",
    "sagar": "सागर",
    "satna": "सतना",
    "sehore": "सीहोर",
    "seoni": "सिवनी",
    "shahdol": "शहडोल",
    "shajapur": "शाजापुर",
    "sheopur": "श्योपुर",
    "shivpuri": "शिवपुरी",
    "sidhi": "सीधी",
    "singrauli": "सिंगरौली",
    "tikamgarh": "टीकमगढ़",
    "ujjain": "उज्जैन",
    "umaria": "उमरिया",
    "vidisha": "विदिशा",
    "mauganj": "मऊगंज",
    "maihar": "मैहर",
    "pandhurna": "पांढुर्णा",

    # Rajasthan (50 districts)
    "ajmer": "अजमेर",
    "alwar": "अलवर",
    "anupgarh": "अनूपगढ़",
    "balotra": "बालोतरा",
    "banswara": "बांसवाड़ा",
    "baran": "बारां",
    "barmer": "बाड़मेर",
    "beawar": "ब्यावर",
    "bharatpur": "भरतपुर",
    "bhilwara": "भीलवाड़ा",
    "bikaner": "बीकानेर",
    "bundi": "बूंदी",
    "chittorgarh": "चित्तौड़गढ़",
    "churu": "चूरू",
    "dausa": "दौसा",
    "deeg": "डीग",
    "dholpur": "धौलपुर",
    "didwana-kuchaman": "डीडवाना-कुचामन",
    "dudu": "दूदू",
    "dungarpur": "डूंगरपुर",
    "gangapur city": "गंगापुर सिटी",
    "hanumangarh": "हनुमानगढ़",
    "jaipur": "जयपुर",
    "jaipur rural": "जयपुर ग्रामीण",
    "jaisalmer": "जैसलमेर",
    "jalore": "जालोर",
    "jhalawar": "झालावाड़",
    "jhunjhunu": "झुंझुनू",
    "jodhpur": "जोधपुर",
    "jodhpur rural": "जोधपुर ग्रामीण",
    "karauli": "करौली",
    "kekri": "केकड़ी",
    "khairthal-tijara": "खैरथल-तिजारा",
    "kota": "कोटा",
    "kotputli-behror": "कोटपूतली-बहरोड़",
    "nagaur": "नागौर",
    "neem ka thana": "नीम का थाना",
    "pali": "पाली",
    "phalodi": "फलोदी",
    "pratapgarh": "प्रतापगढ़",
    "rajsamand": "राजसमंद",
    "salumbar": "सलूम्बर",
    "sanchore": "सांचौर",
    "sawai madhopur": "सवाई माधोपुर",
    "shahpura": "शाहपुरा",
    "sikar": "सीकर",
    "sirohi": "सिरोही",
    "sri ganganagar": "श्री गंगानगर",
    "tonk": "टोंक",
    "udaipur": "उदयपुर",

    # Maharashtra (36 districts)
    "ahmednagar": "अहमदनगर",
    "akola": "अकोला",
    "amravati": "अमरावती",
    "aurangabad": "छत्रपति संभाजीनगर (औरंगाबाद)",
    "chhatrapati sambhajinagar": "छत्रपति संभाजीनगर",
    "beed": "बीड",
    "bhandara": "भंडारा",
    "buldhana": "बुलढाणा",
    "chandrapur": "चंद्रपुर",
    "dhule": "धुले",
    "gadchiroli": "गडचिरोली",
    "gondia": "गोंदिया",
    "hingoli": "हिंगोली",
    "jalgaon": "जलगांव",
    "jalna": "जालना",
    "kolhapur": "कोल्हापुर",
    "latur": "लातूर",
    "mumbai city": "मुंबई शहर",
    "mumbai suburban": "मुंबई उपनगर",
    "nagpur": "नागपुर",
    "nanded": "नांदेड़",
    "nandurbar": "नंदुरबार",
    "nashik": "नासिक",
    "osmanabad": "धाराशिव (उस्मानाबाद)",
    "dharashiv": "धाराशिव",
    "palghar": "पालघर",
    "parbhani": "परभणी",
    "pune": "पुणे",
    "raigad": "रायगढ़",
    "ratnagiri": "रत्नागिरी",
    "sangli": "सांगली",
    "satara": "सतारा",
    "sindhudurg": "सिंधुदुर्ग",
    "solapur": "सोलापुर",
    "thane": "ठाणे",
    "wardha": "वर्धा",
    "washim": "वाशिम",
    "yavatmal": "यवतमाल",

    # West Bengal (23 districts)
    "alipurduar": "अलीपुरद्वार",
    "bankura": "बांकुड़ा",
    "birbhum": "बीरभूम",
    "coochbehar": "कूचबिहार",
    "dakshin dinajpur": "दक्षिण दिनाजपुर",
    "darjeeling": "दार्जिलिंग",
    "hooghly": "हुगली",
    "howrah": "हावड़ा",
    "jalpaiguri": "जलपाईगुड़ी",
    "jhargram": "झाड़ग्राम",
    "kalimpong": "कालिम्पोंग",
    "kolkata": "कोलकाता",
    "malda": "मालदा",
    "murshidabad": "मुर्शिदाबाद",
    "nadia": "नादिया",
    "north 24 parganas": "उत्तर 24 परगना",
    "paschim bardhaman": "पश्चिम बर्धमान",
    "paschim medinipur": "पश्चिम मेदिनीपुर",
    "purba bardhaman": "पूर्व बर्धमान",
    "purba medinipur": "पूर्व मेदिनीपुर",
    "purulia": "पुरुलिया",
    "south 24 parganas": "दक्षिण 24 परगना",
    "uttar dinajpur": "उत्तर दिनाजपुर",

    # Bihar (38 districts)
    "araria": "अररिया",
    "arwal": "अरवल",
    "aurangabad": "औरंगाबाद",
    "banka": "बांका",
    "begusarai": "बेगूसराय",
    "bhagalpur": "भागलपुर",
    "bhojpur": "भोजपुर",
    "buxar": "बक्सर",
    "darbhanga": "दरभंगा",
    "east champaran": "पूर्वी चंपारण",
    "gaya": "गया",
    "gopalganj": "गोपालगंज",
    "jamui": "जमुई",
    "jehanabad": "जहानाबाद",
    "kaimur": "कैमूर",
    "katihar": "कटिहार",
    "khagaria": "खगड़िया",
    "kishanganj": "किशनगंज",
    "lakhisarai": "लखीसराय",
    "madhepura": "मधेपुरा",
    "madhubani": "मधुबनी",
    "munger": "मुंगेर",
    "muzaffarpur": "मुजफ्फरपुर",
    "nalanda": "नालंदा",
    "nawada": "नवादा",
    "patna": "पटना",
    "purnia": "पूर्णिया",
    "rohtas": "रोहतास",
    "saharsa": "सहरसा",
    "samastipur": "समस्तीपुर",
    "saran": "सारण",
    "sheikhpura": "शेखपुरा",
    "sheohar": "शिवहर",
    "sitamarhi": "सीतामढ़ी",
    "siwan": "सीवान",
    "supaul": "सुपौल",
    "vaishali": "वैशाली",
    "west champaran": "पश्चिम चंपारण",

    # Gujarat (33 districts)
    "ahmedabad": "अहमदाबाद",
    "amreli": "अमरेली",
    "anand": "आणंद",
    "aravalli": "अरावली",
    "banaskantha": "बनासकांठा",
    "bharuch": "भरूच",
    "bhavnagar": "भावनगर",
    "botad": "बोटाद",
    "chhota udepur": "छोटा उदेपुर",
    "dahod": "दाहोद",
    "dang": "डांग",
    "devbhoomi dwarka": "देवभूमि द्वारका",
    "gandhinagar": "गांधीनगर",
    "gir somnath": "गिर सोमनाथ",
    "jamnagar": "जामनगर",
    "junagadh": "जूनागढ़",
    "kheda": "खेड़ा",
    "kutch": "कच्छ",
    "mahisagar": "महीसागर",
    "mehsana": "मेहसाणा",
    "morbi": "मोरबी",
    "narmada": "नर्मदा",
    "navsari": "नवसारी",
    "panchmahal": "पंचमहाल",
    "patan": "पाटन",
    "porbandar": "पोरबंदर",
    "rajkot": "राजकोट",
    "sabarkantha": "साबरकांठा",
    "surat": "सूरत",
    "surendranagar": "सुरेंद्रनगर",
    "tapi": "तापी",
    "vadodara": "वडोदरा",
    "valsad": "वलसाड",

    # Punjab (23 districts)
    "amritsar": "अमृतसर",
    "barnala": "बरनाला",
    "bathinda": "बठिंडा",
    "faridkot": "फरीदकोट",
    "fatehgarh sahib": "फतेहगढ़ साहिब",
    "fazilka": "फाजिल्का",
    "ferozepur": "फिरोजपुर",
    "gurdaspur": "गुरदासपुर",
    "hoshiarpur": "होशियारपुर",
    "jalandhar": "जालंधर",
    "kapurthala": "कपूरथला",
    "ludhiana": "लुधियाना",
    "malerkotla": "मलेरकोटला",
    "mansa": "मानसा",
    "moga": "मोगा",
    "muktsar": "श्री मुक्तसर साहिब",
    "pathankot": "पठानकोट",
    "patiala": "पटियाला",
    "rupnagar": "रूपनगर (रोपड़)",
    "sahibzada ajit singh nagar": "मोहाली (एसएएस नगर)",
    "sangrur": "संगरूर",
    "shahid bhagat singh nagar": "नवांशहर (एसबीएस नगर)",
    "tarn taran": "तरन तारन",

    # Haryana (22 districts)
    "ambala": "अंबाला",
    "bhiwani": "भिवानी",
    "charkhi dadri": "चरखी दादरी",
    "faridabad": "फरीदाबाद",
    "fatehabad": "फतेहाबाद",
    "gurugram": "गुरुग्राम",
    "hisar": "हिसार",
    "jhajjar": "झज्जर",
    "jind": "जींद",
    "kaithal": "कैथल",
    "karnal": "करनाल",
    "kurukshetra": "कुरुक्षेत्र",
    "mahendragarh": "महेंद्रगढ़",
    "nuh": "नूह",
    "palwal": "पलवल",
    "panchkula": "पंचकुला",
    "panipat": "पानीपत",
    "rewari": "रेवाड़ी",
    "rohtak": "रोहतक",
    "sirsa": "सिरसा",
    "sonipat": "सोनीपत",
    "yamunanagar": "यमुनानगर",

    # Karnataka (31 districts)
    "bagalkote": "बागलकोट",
    "ballari": "बल्लारी",
    "belagavi": "बेलगावी (बेलगाम)",
    "bengaluru rural": "बेंगलुरु ग्रामीण",
    "bengaluru urban": "बेंगलुरु शहरी",
    "bidar": "बीदर",
    "chamarajanagara": "चामराजनगर",
    "chikkaballapura": "चिक्कबल्लापुर",
    "chikkamagaluru": "चिकमगलूर",
    "chitradurga": "चित्रदुर्ग",
    "dakshina kannada": "दक्षिण कन्नड़ (मंगलुरु)",
    "davanagere": "दावणगेरे",
    "dharwad": "धारवाड़",
    "gadag": "गदग",
    "hassan": "हासन",
    "haveri": "हावेरी",
    "kalaburagi": "कलबुर्गी (गुलबर्गा)",
    "kodagu": "कोडागु (कूर्ग)",
    "kolar": "कोलार",
    "koppal": "कोप्पल",
    "mandya": "मांड्या",
    "mysuru": "मैसूरु (मैसूर)",
    "raichur": "रायचूर",
    "ramanagara": "रामनगर",
    "shivamogga": "शिवमोग्गा (शिमोगा)",
    "tumakuru": "तुमकुरु (तुमकुर)",
    "udupi": "उडुपी",
    "uttara kannada": "उत्तर कन्नड़ (कारवार)",
    "vijayanagara": "विजयनगर",
    "vijayapura": "विजयपुर (बीजापुर)",
    "yadgir": "यादगीर",

    # Tamil Nadu (38 districts)
    "ariyalur": "अरियालुर",
    "chengalpattu": "चेंगलपट्टू",
    "chennai": "चेन्नई",
    "coimbatore": "कोयंबटूर",
    "cuddalore": "कडलूर",
    "dharmapuri": "धर्मपुरी",
    "dindigul": "डिंडीगुल",
    "erode": "इरोड",
    "kallakurichi": "कल्लाकुरिची",
    "kancheepuram": "कांचीपुरम",
    "kanyakumari": "कन्याकुमारी",
    "karur": "करूर",
    "krishnagiri": "कृष्णागिरि",
    "madurai": "मदुरै",
    "mayiladuthurai": "मयिलादुथुरै",
    "nagapattinam": "नागापट्टिनम",
    "namakkal": "नमक्कल",
    "nilgiris": "नीलगिरि",
    "perambalur": "पेराम्बलुर",
    "pudukkottai": "पुदुक्कोट्टई",
    "ramanathapuram": "रामनाथपुरम",
    "ranipet": "रानीपेट",
    "salem": "सलेम",
    "sivaganga": "शिवगंगा",
    "tenkasi": "तेनकासी",
    "thanjavur": "तंजावुर",
    "theni": "थेनी",
    "thoothukudi": "थूथुकुडी (तूतीकोरिन)",
    "tiruchirappalli": "तिरुचिरापल्ली (त्रिची)",
    "tirunelveli": "तिरुनेलवेली",
    "tirupathur": "तिरुपात्तूर",
    "tiruppur": "तिरुपुर",
    "tiruvallur": "तिरुवल्लूर",
    "tiruvannamalai": "तिरुवन्नामलाई",
    "tiruvarur": "तिरुवारूर",
    "vellore": "वेल्लोर",
    "viluppuram": "विलुप्पुरम",
    "virudhunagar": "विरुधुनगर",

    # Andhra Pradesh (26 districts)
    "alluri sitharama raju": "अल्लूरी सीताराम राजू",
    "anakapalli": "अनकापल्ली",
    "anantapur": "अनंतपुर",
    "annamayya": "अन्नमय्या",
    "bapatla": "बापटला",
    "chittoor": "चित्तूर",
    "dr. b.r. ambedkar konaseema": "डॉ. बी.आर. अंबेडकर कोनासीमा",
    "east godavari": "पूर्वी गोदावरी",
    "eluru": "एलूरु",
    "guntur": "गुंटूर",
    "kakinada": "काकीनाडा",
    "krishna": "कृष्णा (मछलीपट्टनम)",
    "kurnool": "कुरनूल",
    "nandyal": "नंद्याल",
    "ntr": "एनटीआर (विजयवाड़ा)",
    "palnadu": "पलनाडु",
    "parvathipuram manyam": "पार्वतीपुरम मान्यम",
    "prakasam": "प्रकाशम (ओंगोल)",
    "sri potti sriramulu nellore": "श्री पोट्टी श्रीरामुलु नेल्लोर",
    "sri sathya sai": "श्री सत्य साई",
    "srikakulam": "श्रीकाकुलम",
    "tirupati": "तिरुपति",
    "visakhapatnam": "विशाखापत्तनम",
    "vizianagaram": "विजयनगरम",
    "west godavari": "पश्चिम गोदावरी",
    "ysr": "वाईएसआर (कडप्पा)",

    # Telangana (33 districts)
    "adilabad": "आदिलाबाद",
    "bhadradri kothagudem": "भद्राद्री कोठागुडेम",
    "hanumakonda": "हनुमाकोंडा",
    "hyderabad": "हैदराबाद",
    "jagtial": "जगित्याल",
    "jangaon": "जनगांव",
    "jayashankar bhupalpally": "जयशंकर भूपालपल्ली",
    "jogulamba gadwal": "जोगुलम्बा गदवाल",
    "kamareddy": "कामारेड्डी",
    "karimnagar": "करीमनगर",
    "khammam": "खम्मम",
    "kumuram bheem asifabad": "कुमुराम भीम आसिफाबाद",
    "mahabubabad": "महबूबाबाद",
    "mahabubnagar": "महबूबनगर",
    "mancherial": "मंचेरियल",
    "medak": "मेदक",
    "medchal malkajgiri": "मेडचल-मलकाजगिरि",
    "mulugu": "मुलुगु",
    "nagarkurnool": "नागरकुरनूल",
    "nalgonda": "नलगोंडा",
    "narayanpet": "नारायणपेट",
    "nirmal": "निर्मल",
    "nizamabad": "निजामाबाद",
    "peddapalli": "पेद्दापल्ली",
    "rajanna sircilla": "राजन्ना सिरसिला",
    "ranga reddy": "रंगा रेड्डी",
    "sangareddy": "संगारेड्डी",
    "siddipet": "सिद्दीपेट",
    "suryapet": "सूर्यापेट",
    "vikarabad": "विकाराबाद",
    "wanaparthy": "वनपर्थी",
    "warangal": "वारंगल",
    "yadadri bhuvanagiri": "यादाद्री भुवनगिरि",

    # Kerala (14 districts)
    "alappuzha": "अलप्पुझा (अल्लेप्पी)",
    "ernakulam": "एर्नाकुलम (कोच्चि)",
    "idukki": "इडुक्की",
    "kannur": "कन्नूर",
    "kasaragod": "कासरगोड",
    "kollam": "कोल्लम (क्विलोन)",
    "kottayam": "कोट्टायम",
    "kozhikode": "कोझिकोड (कालीकट)",
    "malappuram": "मलप्पुरम",
    "palakkad": "पालक्काड़ (पालघाट)",
    "pathanamthitta": "पथानामथिट्टा",
    "thiruvananthapuram": "तिरुवनंतपुरम (त्रिवेंद्रम)",
    "thrissur": "त्रिशूर",
    "wayanad": "वायनाड",

    # Odisha (30 districts)
    "angul": "अनुगुल",
    "balangir": "बलांगीर",
    "balasore": "बालासोर (बालेश्वर)",
    "bargarh": "बरगढ़",
    "bhadrak": "भद्रक",
    "boudh": "बौध",
    "cuttack": "कटक",
    "deogarh": "देवगढ़",
    "dhenkanal": "ढेंकनाल",
    "gajapati": "गजपति",
    "ganjam": "गंजम (बरहामपुर)",
    "jagatsinghpur": "जगतसिंहपुर",
    "jajpur": "जाजपुर",
    "jharsuguda": "झारसुगुड़ा",
    "kalahandi": "कालाहांडी",
    "kandhamal": "कंधमाल",
    "kendrapara": "केंद्रपाड़ा",
    "kendujhar": "केंदुझर (क्योंझर)",
    "khordha": "खोरधा (भुवनेश्वर)",
    "koraput": "कोरापुट",
    "malkangiri": "मलकानगिरि",
    "mayurbhanj": "मयूरभंज (बारीपदा)",
    "nabarangpur": "नबरंगपुर",
    "nayagarh": "नयागढ़",
    "nuapada": "नुआपाड़ा",
    "puri": "पुरी",
    "rayagada": "रायगड़ा",
    "sambalpur": "संबलपुर",
    "subarnapur": "सुवर्णपुर (सोनपुर)",
    "sundergarh": "सुंदरगढ़ (राउरकेला)",

    # Chhattisgarh (33 districts)
    "balod": "बालोद",
    "baloda bazar": "बलौदा बाजार",
    "balrampur": "बलरामपुर",
    "bastar": "बस्तर (जगदलपुर)",
    "bemetara": "बेमेतरा",
    "bijapur": "बीजापुर",
    "bilaspur": "बिलासपुर",
    "dantewada": "दंतेवाड़ा",
    "dhamtari": "धमतरी",
    "durg": "दुर्ग",
    "gariaband": "गरियाबंद",
    "gaurela pendra marwahi": "गौरेला-पेंड्रा-मरवाही",
    "janjgir-champa": "जांजगीर-चांपा",
    "jashpur": "जशपुर",
    "kabirdham": "कबीरधाम (कवर्धा)",
    "kanker": "कांकेर",
    "khairagarh chhuikhadan gandai": "खैरागढ़-छुईखदान-गंडई",
    "kondagaon": "कोंडागांव",
    "korba": "कोरबा",
    "koriya": "कोरिया",
    "mahasamund": "महासमुंद",
    "manendragarh chirmiri bharatpur": "मनेंद्रगढ़-चिरमिरी-भरतपुर",
    "mohla manpur ambagarh chowki": "मोहला-मानपुर-अंbagarh चौकी",
    "mungeli": "मुंगेली",
    "narayanpur": "नारायणपुर",
    "raigarh": "रायगढ़",
    "raipur": "रायपुर",
    "rajnandgaon": "राजनांदगांव",
    "sarangarh bilaigarh": "सारंगढ़-बिलाईगढ़",
    "sakti": "सक्ती",
    "sukma": "सुकमा",
    "surajpur": "सूरजपुर",
    "surguja": "सरगुजा (अंबिकापुर)",

    # Jharkhand (24 districts)
    "bokaro": "बोकारो",
    "chatra": "चतरा",
    "deoghar": "देवघर",
    "dhanbad": "धनबाद",
    "dumka": "दुमका",
    "east singhbhum": "पूर्वी सिंहभूम (जमशेदपुर)",
    "garhwa": "गढ़वा",
    "giridih": "गिरिडीह",
    "godda": "गोड्डा",
    "gumla": "गुमला",
    "hazaribagh": "हजारीबाग",
    "jamtara": "जामताड़ा",
    "khunti": "खूंटी",
    "koderma": "कोडरमा",
    "latehar": "लातेहार",
    "lohardaga": "लोहरदगा",
    "pakur": "पाकुड़",
    "palamu": "पलामू (मेदिनीनगर)",
    "ramgarh": "रामगढ़",
    "ranchi": "राँची",
    "sahibganj": "साहिबगंज",
    "saraikela kharsawan": "सरायकेला खरसावां",
    "simdega": "सिमडेगा",
    "west singhbhum": "पश्चिम सिंहभूम (चाईबासा)",

    # Assam (35 districts)
    "baksa": "बक्सा",
    "barpeta": "बरपेटा",
    "biswanath": "विश्वनाथ",
    "bongaigaon": "बोंगाईगांव",
    "cachar": "कछार (सिलचर)",
    "charaideo": "चराईदेव",
    "chirang": "चिरांग",
    "darrang": "दरंग (मंगलदोई)",
    "dhemaji": "धेमाजी",
    "dhubri": "धुबरी",
    "dibrugarh": "डिब्रूगढ़",
    "dima hasao": "दिमा हसाओ (हाफलोंग)",
    "goalpara": "गोलपारा",
    "golaghat": "गोलाघाट",
    "hailakandi": "हैलाकांडी",
    "hojai": "होजाई",
    "jorhat": "जोरहाट",
    "kamrup": "कामरूप",
    "kamrup metropolitan": "कामरूप महानगर (गुवाहाटी)",
    "karbi anglong": "कार्बी आंगलोंग (डिफू)",
    "karimganj": "करीमगंज",
    "kokrajhar": "कोकराझार",
    "lakhimpur": "लखीमपुर (उत्तरी लखीमपुर)",
    "majuli": "माजुली",
    "morigaon": "मोरीगांव",
    "nagaon": "नागांव",
    "nalbari": "नलबाड़ी",
    "sivasagar": "शिवसागर",
    "sonitpur": "सोनितपुर (तेजपुर)",
    "south salmara-mankachar": "दक्षिण सलमारा-मानकाचर",
    "tinsukia": "तिनसुकिया",
    "udalguri": "उदलगुड़ी",
    "west karbi anglong": "पश्चिम कार्बी आंगलोंग",
    "tamulpur": "तामुलपुर",
    "bajali": "बजाली",

    # Himachal Pradesh (12 districts)
    "bilaspur": "बिलासपुर",
    "chamba": "चंबा",
    "hamirpur": "हमीरपुर",
    "kangra": "कांगड़ा (धर्मशाला)",
    "kinnaur": "किन्नौर (रेकॉन्ग पियो)",
    "kullu": "कुल्लू",
    "lahaul and spiti": "लाहौल और स्पीति (केलांग)",
    "mandi": "मंडी",
    "shimla": "शिमला",
    "sirmaur": "सिरमौर (नाहन)",
    "solan": "सोलन",
    "una": "ऊना",

    # Uttarakhand (13 districts)
    "almora": "अल्मोड़ा",
    "bageshwar": "बागेश्वर",
    "chamoli": "चमोली (गोपेश्वर)",
    "champawat": "चंपावत",
    "dehradun": "देहरादून",
    "haridwar": "हरिद्वार",
    "nainital": "नैनीताल",
    "pauri garhwal": "पौड़ी गढ़वाल",
    "pithoragarh": "पिथौरागढ़",
    "rudraprayag": "रुद्रप्रयाग",
    "tehri garhwal": "टिहरी गढ़वाल",
    "udham singh nagar": "उधम सिंह नगर (रुद्रपुर)",
    "uttarkashi": "उत्तरकाशी",

    # Jammu and Kashmir (20 districts)
    "anantnag": "अनंतनाग",
    "bandipora": "बांदीपोरा",
    "baramulla": "बारामूला",
    "budgam": "बडगाम",
    "doda": "डोडा",
    "ganderbal": "गांदरबल",
    "jammu": "जम्मू",
    "kathua": "कठुआ",
    "kishtwar": "किश्तवाड़",
    "kulgam": "कुलगाम",
    "kupwara": "कुपवाड़ा",
    "poonch": "पुंछ",
    "pulwama": "पुलवामा",
    "rajouri": "राजौरी",
    "ramban": "रामबन",
    "reasi": "रियासी",
    "samba": "सांबा",
    "shopian": "शोपियां",
    "srinagar": "श्रीनगर",
    "udhampur": "उधमपुर",

    # Ladakh (2 districts)
    "leh": "लेह",
    "kargil": "कारगिल",

    # Goa (2 districts)
    "north goa": "उत्तर गोवा (पणजी)",
    "south goa": "दक्षिण गोवा (मडगांव)",

    # Delhi (11 districts)
    "central delhi": "मध्य दिल्ली",
    "east delhi": "पूर्वी दिल्ली",
    "new delhi": "नई दिल्ली",
    "north delhi": "उत्तरी दिल्ली",
    "north east delhi": "उत्तर पूर्वी दिल्ली",
    "north west delhi": "उत्तर पश्चिमी दिल्ली",
    "shahdara": "शाहदरा",
    "south delhi": "दक्षिणी दिल्ली",
    "south east delhi": "दक्षिण पूर्वी दिल्ली",
    "south west delhi": "दक्षिण पश्चिमी दिल्ली",
    "west delhi": "पश्चिमी दिल्ली",

    # UTs: Chandigarh, Puducherry, A&N, DNH&DD, Lakshadweep
    "chandigarh": "चंडीगढ़",
    "puducherry": "पुडुचेरी",
    "karaikal": "कराईकल",
    "mahe": "माहे",
    "yanam": "यानम",
    "nicobar": "निकोबार",
    "north and middle andaman": "उत्तर और मध्य अंडमान",
    "south andaman": "दक्षिण अंडमान (पोर्ट ब्लेयर)",
    "dadra and nagar haveli": "दादरा और नगर हवेली (सिलवासा)",
    "daman": "दमन",
    "diu": "दीव",
    "lakshadweep": "लक्षद्वीप (कवरत्ती)",

    # North East States: Tripura, Manipur, Meghalaya, Mizoram, Nagaland, Sikkim, Arunachal
    # Tripura (8)
    "dhalai": "धलाई (अम्बासा)",
    "gomati": "गोमती (उदयपुर)",
    "khowai": "खोवाई",
    "north tripura": "उत्तर त्रिपुरा (धर्मनगर)",
    "sepahijala": "सिपाहीजाला (विशालगढ़)",
    "south tripura": "दक्षिण त्रिपुरा (बेलोनिया)",
    "unakoti": "उनाकोटी (कैलाशहर)",
    "west tripura": "पश्चिम त्रिपुरा (अगरतला)",

    # Meghalaya (12)
    "east garo hills": "पूर्वी गारो हिल्स",
    "east jaintia hills": "पूर्वी जयंतिया हिल्स",
    "east khasi hills": "पूर्वी खासी हिल्स (शिलांग)",
    "eastern west khasi hills": "ईस्टर्न वेस्ट खासी हिल्स",
    "north garo hills": "उत्तरी गारो हिल्स",
    "ri bhoi": "री भोई (नोंगपोह)",
    "south garo hills": "दक्षिणी गारो हिल्स (बाघमारा)",
    "south west garo hills": "दक्षिण पश्चिम गारो हिल्स",
    "south west khasi hills": "दक्षिण पश्चिम खासी हिल्स",
    "west garo hills": "पश्चिम गारो हिल्स (तुरा)",
    "west jaintia hills": "पश्चिम जयंतिया हिल्स (जोवाई)",
    "west khasi hills": "पश्चिम खासी हिल्स (नोंगस्टोइन)",

    # Manipur (16)
    "bishnupur": "बिष्णुपुर",
    "chandel": "चंदेल",
    "churachandpur": "चुराचांदपुर",
    "imphal east": "इंफाल पूर्व",
    "imphal west": "इंफाल पश्चिम",
    "jiribam": "जिरीबाम",
    "kakching": "काकचिंग",
    "kamjong": "कामजोंग",
    "kangpokpi": "कांगपोकपी",
    "noney": "नोने",
    "pherzawl": "फेरज़ोल",
    "senapati": "सेनापति",
    "tamenglong": "तमेंगलोंग",
    "tengnoupal": "तेंगनौपाल",
    "thoubal": "थौबल",
    "ukhrul": "उखरुल",

    # Mizoram (11)
    "aizawl": "आइजोल",
    "champhai": "चम्फाई",
    "hnathial": "ह्नाथियाल",
    "khawzawl": "खावजोल",
    "kolasib": "कोलासिब",
    "lawngtlai": "लोंगतलाई",
    "lunglei": "लुंगलेई",
    "mamit": "मामित",
    "saitual": "सैतुअल",
    "serchhip": "सेरछिप",
    "siaha": "सियाहा",

    # Nagaland (16)
    "chumoukedima": "चुमौकेदिमा",
    "dimapur": "दीमापुर",
    "kiphire": "किफिरे",
    "kohima": "कोहिमा",
    "longleng": "लोंगलेंग",
    "mokokchung": "मोकोकचुंग",
    "mon": "मोन",
    "niuland": "नियूलैंड",
    "noklak": "नोकलाक",
    "peren": "पेरेन",
    "phek": "फेक",
    "shamator": "शामटोर",
    "tsminyu": "त्सेमिन्यु",
    "tuensang": "तुएनसांग",
    "wokha": "वोखा",
    "zunheboto": "जुन्हेबोटो",

    # Sikkim (6)
    "gangtok": "गंगटोक (पूर्वी सिक्किम)",
    "gyalshing": "ग्यालशिंग (पश्चिम सिक्किम)",
    "pakyong": "पाकयोंग",
    "mangan": "मंगन (उत्तरी सिक्किम)",
    "namchi": "नामची (दक्षिण सिक्किम)",
    "soreng": "सोरेन्ग",

    # Arunachal Pradesh (26)
    "anjaw": "अंजाव (हवाई)",
    "changlang": "चांगलांग",
    "dibang valley": "दिबांग घाटी (अनिनी)",
    "east kameng": "पूर्वी कामेंग (सेप्पा)",
    "east siang": "पूर्वी सियांग (पासीघाट)",
    "itangar capital complex": "ईटानगर",
    "kamle": "कमले (रागा)",
    "kra daadi": "क्रा दादी (जमीन)",
    "kurung kumey": "कुरुंग कुमे (कोलोरियांग)",
    "lepa rada": "लेपा राडा (बासर)",
    "lohit": "लोहित (तेजू)",
    "longding": "लॉन्गडिंग",
    "lower dibang valley": "निचली दिबांग घाटी (रोइंग)",
    "lower siang": "निचला सियांग (लिकाबाली)",
    "lower subansiri": "निचला सुबनसिरी (जीरो)",
    "namsai": "नामसाई",
    "pakke kessang": "पक्के केशांग (लेम्मी)",
    "papum pare": "पपुम पारे (युपिया)",
    "shi yomi": "शी योमी (तातो)",
    "siang": "सियांग (पांगिन)",
    "tawang": "तवांग",
    "tirap": "तिराप (खोंसा)",
    "upper siang": "ऊपरी सियांग (यिंगकियोंग)",
    "upper subansiri": "ऊपरी सुबनसिरी (दापोरिजो)",
    "west kameng": "पश्चिम कामेंग (बोमडिला)",
    "west siang": "पश्चिम सियांग (आलो)",
    "itanagar": "ईटानगर",
    "janjgir champa": "जांजगीर-चांपा",
    "devbhumi dwarka": "देवभूमि द्वारका",
    "ri-bhoi": "री भोई",
    "hnahthial": "ह्नाथियाल",
    "saiha": "सियाहा",
    "tseminyu": "त्सेमिन्यु",
    "sundargarh": "सुंदरगढ़",
    "mohali": "मोहाली",
    "nawanshahr": "नवांशहर",
    "didwana kuchaman": "डीडवाना-कुचामन",
    "khairthal tijara": "खैरथल-तिजारा",
    "kotputli behror": "कोटपूतली-बहरोड़",
    "salumber": "सलूम्बर",
    "kanchipuram": "कांचीपुरम",
    "cooch behar": "कूचबिहार",
    "ananthapuramu": "अनंतपुर",
    "ysr kadapa": "वाईएसआर (कडप्पा)",
    "chhota udaipur": "छोटा उदेपुर",
    "kheri": "लखीमपुर खीरी"
}

def generate():
    with open('src/lib/districtsCatalog.json', 'r', encoding='utf-8') as f:
        catalog = json.load(f)

    result = {}
    missing = []

    for item in catalog:
        dist_raw = item['district_name']
        dist_key = dist_raw.lower().strip()

        if dist_key in MANUAL_DISTRICT_HINDI:
            result[dist_key] = MANUAL_DISTRICT_HINDI[dist_key]
        else:
            # Check partial key
            matched = False
            for k, v in MANUAL_DISTRICT_HINDI.items():
                if k in dist_key or dist_key in k:
                    result[dist_key] = v
                    matched = True
                    break
            if not matched:
                missing.append(dist_raw)

    print(f"Total catalog districts: {len(catalog)}")
    print(f"Matched in Hindi: {len(result)}")
    print(f"Missing count: {len(missing)}")
    if missing:
        print("Sample missing:", missing[:20])

    # Save to JSON
    with open('src/i18n/districtsHindi.json', 'w', encoding='utf-8') as out:
        json.dump(MANUAL_DISTRICT_HINDI, out, ensure_ascii=False, indent=2)
    print("Saved src/i18n/districtsHindi.json successfully!")

if __name__ == '__main__':
    generate()
